        ...
        for (int i = 0; i < countBeforeAdding; i++)
        {
            ...
            switch (TilePrefabs[i].Rotation)
            {
                ...
                case VoxelTile.RotationType.FourRotations:
                    ...
                    TilePrefabs.Add(clone);clone = Instantiate(TilePrefabs[i], position: TilePrefabs[i].transform.position + Vector3.right * 2, Quaternion.identity);
                    clone.Rotate90();
                    clone.Rotate90();
                    TilePrefabs.Add(clone);

                    TilePrefabs.Add(clone);clone = Instantiate(TilePrefabs[i], position: TilePrefabs[i].transform.position + Vector3.right * 3, Quaternion.identity);
                    clone.Rotate90();
                    clone.Rotate90();
                    clone.Rotate90();
                    TilePrefabs.Add(clone);
